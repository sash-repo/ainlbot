...
# # api_post from handler.py, copied for testing purposes but should be integrated properly
...
def retrieve_table_data(datasource_names):
    ...
    #     ]
    # }
    # This dictionary should make forming an informative message for the user easier (e.g. "{arg1} from {table1} in {DataSource-2} is abnormally high")
...
async def send_nl_prompt(kpi_arg):
    ...
    # Check that the KPI arg has a relevant caculation (sum or average)
...
def detect_anomalies(df, corridors):
    ...
    # Loop values, if value is outside calculated quartile boundries, append detected anomalies to list
...
async def perform_anomaly_check():
    ...
    # Connect to the user's database - for testing purposes - (Likely should be integrated with existing functions in connectors.py)
    ...
    for data_source, tables in table_data.items():
        for table in tables:
            for kpi in table['kpis']:
                ...
                trusted_result = await conn.fetch(trusted_sql['sql'])
                comparison_result = await conn.fetch(comparison_sql['sql'])
                ...
                # Form message to send user is anomalies have been detected
                if anomalies:
                    ...
                    if EMAIL_ADDRESS != '' and EMAIL_PASSWORD != '' and RECIPIENT_EMAIL != '':
                        try:
                            send_email(anomaly_message)
                        except:
                            logging.info("There was a problem sending the email")
...
# TODO: Add error handling with try/except etc.
